...
def compile_global_func_def(func: CheckedFunction, def_node: DFContainingVNode, graph: Hugr, globals: CompiledGlobals) -> CompiledFunctionDef:
    ...
    graph.add_output(inputs=[cfg_node.add_out_port(ty) for ty in type_to_row(func.cfg.output_ty)], parent=def_node)

    return CompiledFunctionDef(func.name, func.ty, func.defined_at, None, def_node)


def compile_local_func_def(func: CheckedNestedFunctionDef, dfg: DFContainer, graph: Hugr, globals: CompiledGlobals) -> PortVariable:
    ...
    if len(captured) > 0 and func.name in func.cfg.live_before[func.cfg.entry_bb]:
        ...
        partial = graph.add_partial(loaded, [def_input.out_port(i) for i in range(len(captured))], def_node)
        ...
        globals = globals | {func.name: CompiledFunctionDef(func.name, func.ty, func, None, def_node)}
    ...
    if len(captured) > 0:
        loaded = graph.add_partial(loaded, [dfg[v.name].port for v in captured], dfg.node).out_port(0)

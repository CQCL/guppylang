...
def compile_qsystem_guppy(fn) -> ModulePointer:  # type: ignore[no-untyped-def]
    ...
    module.load(angle, qubit, get_current_shot, RNG, make_discrete_distribution, measure_array)  # type: ignore[arg-type]
...
def test_qsystem_random(validate):  # type: ignore[no-untyped-def]
    ...
    @compile_qsystem_guppy
    def test() -> tuple[int, float, int, int, angle]:
        rng = RNG(42)
        rint = rng.random_int()
        ...
        return rint, rfloat, rint_bnd, rint_discrete, rangle

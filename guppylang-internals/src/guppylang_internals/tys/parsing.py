...
def _parse_callable_type(
    ...
    inputs = [
        parse_function_arg_annotation(
            inp, None, globals, param_var_mapping, allow_free_vars
        )
        for inp in inputs.elts
    ]
    output = type_from_ast(output, globals, param_var_mapping, allow_free_vars)
...
def parse_function_arg_annotation(
    ...
    ty, flags = type_with_flags_from_ast(
        annotation, globals, param_var_mapping, allow_free_vars
    )
    return check_function_arg(ty, flags, annotation, name, param_var_mapping)

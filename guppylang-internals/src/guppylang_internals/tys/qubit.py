...
def qubit_ty() -> Type:
    """Returns the qubit type. Beware that this function imports guppylang definitions, so
    if called before the definitions are registered, it will result in circular imports.
    """
...
def is_qubit_ty(ty: Type) -> bool:
    ...
    This function results in circular imports if called before qubit types are registered.
    """
    return ty == qubit_ty()

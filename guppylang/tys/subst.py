...
from guppylang.tys.ty import GuppyType, ExistentialTypeVar, BoundTypeVar, FunctionType
...
Subst = dict[ExistentialVar, GuppyType]  # TODO: `GuppyType | Const` or `Argument` ??
...
class Substituter(Transformer):
    ...
    @transform.register
    def _transform_ExistentialTypeVar(self, ty: ExistentialTypeVar) -> GuppyType | None:
        return self.subst.get(ty, None)

    @transform.register
    def _transform_ExistentialConstVar(self, ty: ExistentialConstVar) -> GuppyType | None:
...
class Instantiator(Transformer):
    ...
    @transform.register
    def _transform_BoundTypeVar(self, ty: BoundTypeVar) -> GuppyType | None:
    ...
    @transform.register
    def _transform_BoundConstVar(self, ty: BoundConstVar) -> GuppyType | None:
        raise NotImplementedError

    @transform.register
    def _transform_FunctionType(self, ty: FunctionType) -> GuppyType | None:
        if ty.parametrized:
